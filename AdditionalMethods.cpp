 ...
 * Method generates random file name and adds to the prefix
 */
std::string AdditionalMethods::generateFileName2()
{
    time_t t;
    time(&t);
    srand((unsigned int)(t+rand()));
    ...
    std::string catString, path;
    path.assign(AdditionalMethods::inputDataFile2);

    std::string::size_type tt = path.find_last_of("/");
    path = path.substr(0,tt) + "/";
    AdditionalMethods::tempPath2.assign(path);
    catString.assign(AdditionalMethods::tempPath2);
